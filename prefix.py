...
def prefix_code_for_lengths(lengths):
    ...
    if any(length <= 0 for length in lengths):
        raise ValueError("prefix-code lengths must be positive integers")

    if sum(2 ** -length for length in lengths) > 1:
        raise ValueError("prefix-code lengths must satisfy the Kraft inequality")

    if not lengths:
        return []

    lengths_heap = list(lengths)
    heapq.heapify(lengths_heap)

    next_length = heapq.heappop(lengths_heap)
    leaves = []
